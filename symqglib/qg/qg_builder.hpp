...
class QGBuilder {
   ...
   public:
    explicit QGBuilder(
        QuantizedGraph& index, uint32_t ef_build, const float* data, size_t num_threads, const float* centroids
    )
        : qg_{index}
        , ef_build_{ef_build}
...
#pragma omp parallel for
    for (size_t i = 0; i < num_nodes_; ++i) {
        ...
        neighbor_set.reserve(std::min(degree_bound_, num_nodes_ - 1));
        while (neighbor_set.size() < num_nodes_ - 1 && 
               neighbor_set.size() < degree_bound_) {
        ...
        std::vector<PID> neighbor_vec(neighbor_set.begin(), neighbor_set.end());
        for(size_t j = 0; j< degree_bound_; ++j) {
            new_neighbors_[i].emplace_back(
                neighbor_vec[j%neighbor_vec.size()]
                , dist_func_(cur_data, qg_.get_vector(neighbor_vec[j%neighbor_vec.size()]), dim_)
            );
        }
    }
    std::cout << "Random initialization finished.\n";
        ...
        if (new_result.size() < degree_bound_) {
            ...
            if (new_result.size() < degree_bound_) {
                ...
                while (new_result.size() < degree_bound_) {
                    ...
                    if (rand_id != static_cast<PID>(i) && (num_nodes_ - 1 < degree_bound_ ||
