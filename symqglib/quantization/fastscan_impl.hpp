...
// 新的编码函数：直接产生正确的字节顺序，避免解码时的反序操作
inline void pack_codes_ordered(
    size_t padded_dim, const uint64_t* binary_code, size_t ncode, uint64_t* ordered_codes
) {
    size_t code_len = padded_dim >> 6;  // 每个向量需要的uint64_t数量
    
    for (size_t code_idx = 0; code_idx < ncode; ++code_idx) {
        for (size_t i = 0; i < code_len; ++i) {
            uint64_t code = binary_code[code_idx * code_len + i];
            ...
            ordered_codes[code_idx * code_len + i] = ordered_code;
        }
    }
}
...
inline void accumulate_impl(
    size_t dim,
    const uint64_t* __restrict__ binary_codes,
...
//     abort();
// #endif
    // 普通实现版本，直接处理uint64_t格式的量化码
    // 初始化结果数组
    for (size_t i = 0; i < num_codes && i < kBatchSize; ++i) {
        result[i] = 0;
    }
    size_t code_len = dim >> 6;
    
    // 处理每个量化码
    for (size_t code_idx = 0; code_idx < num_codes && code_idx < kBatchSize; ++code_idx) {
        uint16_t accumulator = 0;
        // 将64位码分解为16个4位的子码，每个子码作为LUT的索引
        size_t lut_base = 0;
        for(size_t i = 0; i < code_len; ++i) {
            uint64_t code = binary_codes[code_idx * code_len + i];
        
            // 模拟pack_codes中的字节序反序和4位交换
            // 首先将64位码转换为8个字节
            uint8_t bytes[8];
            for (int b = 0; b < 8; ++b) {
                bytes[b] = (code >> (b * 8)) & 0xFF;
            ...
            // 现在从交换后的字节中提取4位码
            for (int b = 0; b < 8; ++b) {
                if (lut_base >= dim * 4) break;
                
                // 每个字节包含2个4位码
                uint8_t lo_4bit = bytes[b] & 0x0F;        // 低4位
                uint8_t hi_4bit = (bytes[b] >> 4) & 0x0F; // 高4位
                
                // 先处理低4位
                if (lut_base < dim * 4) {
                    accumulator += LUT[lut_base + lo_4bit];
                    lut_base += 16;
                }
                
                // 再处理高4位
                if (lut_base < dim * 4) {
                    accumulator += LUT[lut_base + hi_4bit];
                    lut_base += 16;
                }
            }
            // 按字节输出调整前后的code的16个4位码
            // std::cout << "bytes: ";
            // for (int b = 0; b < 8; ++b) {
            //     std::cout << std::hex << (int)bytes[b] << " ";
            // }
        ...
        result[code_idx] = accumulator;
    }
    // 比较 result 和 result_normal
    // for (size_t i = 0; i < num_codes && i < kBatchSize; ++i) {
    //     if (result[i] != result_normal[i]) {
    //         std::cerr << "Mismatch at index " << i << ": SIMD result = " << result[i]
    //                   << ", Normal result = " << result_normal[i] << std::endl;
    //         // abort();
    //     }
    // }
}
...
// pack float lut for fast scan, dim % 4 == 0
inline void pack_float_lut_impl(
    size_t dim, const float* __restrict__ float_query, float* __restrict__ float_LUT
) {
    size_t num_codebook = dim >> 2;
    for (size_t i = 0; i < num_codebook; ++i) {
        float_LUT[0] = 0.0f;
        for (int j = 1; j < 16; ++j) {
            float_LUT[j] = float_LUT[j - LOWBIT(j)] + float_query[kPos[j]];
        }
        float_LUT += 16;
        float_query += 4;
    }
...
// float version of accumulate_impl using float LUT
inline void accumulate_float_impl(
    size_t dim,
    const uint64_t* __restrict__ binary_codes,
    ...
    for (size_t code_idx = 0; code_idx < num_codes && code_idx < kBatchSize; ++code_idx) {
        ...
        for(size_t i = 0; i < code_len; ++i) {
            ...
            for (int b = 0; b < 8; ++b) {
                ...
                // 使用LUT累加
                accumulator += float_LUT[lut_base + lo4];
                lut_base += 16;
                if (lut_base < dim * 4) {
                    accumulator += float_LUT[lut_base + hi4];
                    lut_base += 16;
                }
            }
        }
...
// 简化的float版本：直接处理已排序的编码数据，无需字节反序和4位交换
inline void accumulate_float_impl_ordered(
    size_t dim,
    const uint64_t* __restrict__ ordered_binary_codes,
    const float* __restrict__ float_LUT,
    float* __restrict__ result,
    size_t num_codes
) {
    ...
    for (size_t code_idx = 0; code_idx < num_codes && code_idx < kBatchSize; ++code_idx) {
        ...
        for (size_t i = 0; i < code_len; ++i) {
            uint64_t code = ordered_binary_codes[code_idx * code_len + i];
            
            // 直接从已排序的码中提取4位子码，无需反序操作
            uint8_t bytes[8];
            for (int b = 0; b < 8; ++b) {
                bytes[b] = (code >> (b * 8)) & 0xFF;
            }
