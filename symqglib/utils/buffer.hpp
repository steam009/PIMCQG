...
class ResultBuffer {
   public:
    ...
    const std::vector<float, memory::AlignedAllocator<float>>& distances() { return distances_; }
    size_t size() const { return size_; }


    void copy_results(PID* knn, size_t num) const { 
        size_t copy_count = std::min(num, size_);
        std::copy(ids_.begin(), ids_.begin() + copy_count, knn); 
    }
