...
typedef struct {
    ...
    bool overflow_full;    /* true when overflow_table is exhausted;
                              hashset_get returns true for any collision
                              to prevent re-visitation loops (2nd defense line) */
} hashset_t;
...
static inline bool search_buffer_has_next(search_buffer_t *buf);

/* Reset capacity */
static inline void search_buffer_resize(search_buffer_t *buf, candidate_t *new_data_array, size_t new_capacity);

/* Merge multiple tasklet-local SearchBuffers into main SearchBuffer */
static inline void merge_search_buffer(search_buffer_t *main_buf, search_buffer_t *local_bufs, size_t num_bufs);
...
static inline bool search_buffer_has_next(search_buffer_t *buf) {
    // Skip elements already marked by set_checked
    while (buf->cur < buf->size && is_checked(buf->data[buf->cur].id)) {
        buf->cur++;
    }
...
static inline void merge_search_buffer(search_buffer_t *main_buf, search_buffer_t *local_bufs, size_t num_bufs) {
    for (size_t i = 0; i < num_bufs; i++) {
        for (size_t j = 0; j < local_bufs[i].size; j++) {
            if (!search_buffer_is_full(main_buf, local_bufs[i].data[j].distance)) {
                search_buffer_insert(main_buf, 
                                    local_bufs[i].data[j].id, 
                                    local_bufs[i].data[j].distance);
            }
        }
        /* Clear local buffer for next use */
        search_buffer_clear(&local_bufs[i]);
    }
}
...
static inline bool hashset_get(const hashset_t *set, PID data_id) {
    ...
    if ((unsigned int)val != KPID_MAX && set->overflow_table != NULL) {
        ...
        /* overflow_full: overflow table exhausted, treat all colliding nodes as
         * visited to prevent re-visitation loops. This is the 1st defense line;
         * MAX_BEAM_ITERS acts as the 2nd defense line. */
        if (set->overflow_full) {
            return true;
        }
    }
...
static inline void hashset_set(hashset_t *set, PID data_id) {
    ...
    if (set->overflow_table != NULL) {
        ...
        if (set->overflow_count < set->overflow_size) {
            ...
            if (set->overflow_count == set->overflow_size) {
                set->overflow_full = true;
            }
        } else {
            /* Overflow table already full: set the flag so hashset_get
             * returns true for all future collisions (1st defense line). */
            set->overflow_full = true;
        }
    }
}
